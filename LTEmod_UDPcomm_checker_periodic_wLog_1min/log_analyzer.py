...
List target files, then aggregate which commands precede:
    "EVENT: ERROR detected; GPIO20 LOW; pending restart"

Assumed log examples:
    [2025/12/22,19:05:58+36] CMD: AT%SOCKETCMD="ACTIVATE",1
    [2025/12/22,19:05:58+36] EVENT: ERROR detected; GPIO20 LOW; pending restart

Outputs:
  1) Prints the file list being scanned
  2) Prints a sorted summary: <EVENT count> <command>
  3) Optional detail/summary CSV/Excel files

Usage:
    python aggregate_event_error_gpio20.py
Options:
...
EVENT_LITERAL = "EVENT: ERROR detected; GPIO20 LOW; pending restart"
...
def main():
    parser = argparse.ArgumentParser(description="List files then aggregate CMDs preceding specific EVENT.")
