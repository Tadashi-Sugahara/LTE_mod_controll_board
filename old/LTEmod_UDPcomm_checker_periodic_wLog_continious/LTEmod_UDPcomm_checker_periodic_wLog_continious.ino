 ...
 * - ★ /command1.txt に "WAIT <msec>" 行を記述すると、その msec だけ delay。表示は LCD のみ（ログ・PC出力なし）
 ...
 * - 表示「Next run in ...」は RUN_INTERVAL_MS に追従
 */
#include <Arduino.h>
#include <FS.h>
#include <LittleFS.h>
...
/* ==== タイムアウトを 10 秒に統一 ==== */
const unsigned long COMMAND_RESPONSE_TIMEOUT_MS = 10000UL; // 既定 10s
const unsigned long RUN_INTERVAL_MS = 10000UL; // 次周期までのインターバル（表示追従）
/* ==== コマンド間の待ち時間（200ms → 1000ms） ==== */
const unsigned long INTER_CMD_DELAY_MS = 1000UL;
uint32_t ngCountRun=0, passCountRun=0;
/* ==== ERROR 復帰用 GPIO20 ==== */
unsigned long gpio20LowTime=0; bool gpio20IsLow=false;
const unsigned long GPIO20_LOW_DURATION_MS=1000; // LOW→HIGH 自動復帰まで約 1 秒
...
const unsigned long PROBE_INTERVAL_MS = 500; // 500ms おきに AT 送信（無期限）
/* ==================== GP23: 押すと Low の割り込みスイッチ ==================== */
...
/* ==================== %SOCKETDATA パーサ（送受共通） ==================== */
// 受信例: %SOCKETDATA:<cid>,<len>,<flags>,"<payload>",...
bool parseSocketdataRx(const String& line, int& cid, int& len, int& flags, String& payloadRaw){
  ...
  String prefix=line.substring(p+12, q1); // after "%SOCKETDATA:" up to first quote
  prefix.trim();
...
// 送信例: AT%SOCKETDATA="SEND",<cid>,<len>,"<payload>"
bool parseSocketdataSendCmd(const String& cmd, int& cid, int& len, String& payloadRaw){
  int p=cmd.indexOf("AT%SOCKETDATA=\"SEND\"");
  if(p<0) return false;
  int q2=cmd.lastIndexOf('"'); // last quote = payload closing
  int q1=cmd.lastIndexOf('"', q2-1); // previous quote = payload opening
  if(q1<0 || q2<=q1) return false;
  payloadRaw = cmd.substring(q1+1, q2);
  int c2=cmd.lastIndexOf(',', q1-1); // comma before payload opening -> len separator
  int c1=cmd.lastIndexOf(',', c2-1); // previous comma -> cid separator
...
/* ==================== 送受ペイロードの正規化（比較用） ==================== */
String normalizePayloadForCompare_FromSend(const String& payloadAscii){
  return makePrintable(payloadAscii); // 送信はASCII前提を可視化ルールへ
}
String normalizePayloadForCompare_FromRx(const String& payloadRaw){
  if(isPureHexString(payloadRaw)){
    String bytes = hexToBytesString(payloadRaw);
    return makePrintable(bytes);
  }else{
    return makePrintable(payloadRaw);
  }
}
/* ==================== ATコマンド実行（COPS特別処理＋共通 ERROR リトライ） ==================== */
...
const int COPS_MAX_ATTEMPTS = 10; // リトライ上限（10回）
const unsigned long COPS_TIMEOUT_PER_TRY_MS = 10000UL; // 常に 10s（延長廃止）
/* ==== ERROR リトライ ==== */
...
void sendNextCommand(){
  ...
  // === ★ WAIT 命令（LCD表示のみ／ログ・PC出力なし） ===
  if(command.startsWith("WAIT")){
    int sp = command.indexOf(' ');
    unsigned long waitMs = 0;
    if(sp > 0){
      String msStr = command.substring(sp+1); msStr.trim();
      waitMs = (unsigned long)msStr.toInt();
    }
    // LCDにのみ表示
    lcdClearBeforeNewLine();
    lcdPrintLine(String("WAIT ") + waitMs + " ms...");
    // 実際の待機
    delay(waitMs);
    // 次行へ
    currentLineIndex++;
  ...
  if(isSendCmd){
    String spNorm = normalizePayloadForCompare_FromSend(spayloadRaw);
    if(spNorm.length()>26) spNorm = spNorm.substring(0,26);
    sendDataString = spNorm;
    hasSendData = true;
  ...
  if(command.equals("AT+COPS=1,2,\"44020\"")){
    ...
    copsAttempts = 1;
    copsSawHex = false;
    currentCommandTimeoutMs = COPS_TIMEOUT_PER_TRY_MS; // 10s/試行
    logWrite("COPS: special handling started (timeout=10s per try, no extension)");
  }else{
    isCopsCommandActive = false;
    copsAttempts = 0; copsSawHex = false;
    currentCommandTimeoutMs = COMMAND_RESPONSE_TIMEOUT_MS; // 通常も 10s（統一）
...
void loop(){
  ...
  if(uart1GetLine(resp)){
    ...
    if(parseSocketdataRx(printable, rcid, rlen, rflags, rpayloadRaw)){
      String rpNorm = normalizePayloadForCompare_FromRx(rpayloadRaw);
      if(rpNorm.length()>26) rpNorm = rpNorm.substring(0,26);
      receiveDataString = rpNorm;
      hasReceiveData = true;
